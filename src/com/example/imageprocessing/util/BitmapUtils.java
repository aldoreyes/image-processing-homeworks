...
import com.example.imageprocessing.Border;
import com.example.imageprocessing.Border.BorderPoint;
...
public class BitmapUtils {
	...
	private static boolean kernelMatch(int x, int y, int w, int h,
		...
		for (int i = 0; i < lenv; i++) {
			for (int j = 0; j < lenh; j++) {
				int pVal = ((bitmap.getPixel(x + j, y + i) & 0xff) > 0 ? 1 : 0);
				if (y + i >= h || x + j >= w || se[i][j] != pVal) {
					return false;
				}
			}
		}
